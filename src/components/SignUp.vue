        ...
        <v-card elevation="10" class="pa-6 rounded-lg">
          <v-form @submit.prevent="signUp" ref="form">
            <div class="text-center mb-6">
              <h1 class="text-h4 font-weight-bold teal--text text--darken-2"> Join Shoply </h1>
            </div>
            ...
            <v-text-field v-model="phoneNumber" label="Phone Number" prepend-inner-icon="mdi-phone" :rules="[(v) => !!v || 'Phone is required']" required outlined dense ></v-text-field>
            <v-text-field v-model="deliveryAddress" label="Delivery Address" prepend-inner-icon="mdi-location" :rules="[(v) => !!v || 'Address is required']" required outlined dense ></v-text-field>
